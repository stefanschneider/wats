...
var _ = Describe("Getting instance information", func() {
	BeforeEach(func() {
		Eventually(cf.Cf("push", appName, "-m", "2Gb", "-p", "../assets/webapp", "--no-start", "-b", "https://github.com/stefanschneider/windows_app_lifecycle#buildpack-extraction", "-s", "windows2012R2"), CF_PUSH_TIMEOUT).Should(Exit(0))
		enableDiego(appName)
