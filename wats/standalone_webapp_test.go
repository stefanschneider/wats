...
var _ = Describe("A standalone webapp", func() {
	...
	Describe("staged on Diego and running on Diego", func() {
		It("exercises the app through its lifecycle", func() {
			By("pushing it", func() {
				Eventually(cf.Cf("push", appName, "-p", "../assets/webapp", "--no-start", "-b", "https://github.com/stefanschneider/windows_app_lifecycle#buildpack-extraction", "-s", "win2012r2"), CF_PUSH_TIMEOUT).Should(Exit(0))
			})
